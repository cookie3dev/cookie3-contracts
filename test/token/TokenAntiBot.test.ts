...
describe("Anti-bot", () => {
  ...
  async function makeSUT() {
    return (await deployContract(deployer, TokenArtifacts, [deployer.address])) as Cookie3;
  }

  beforeEach(async () => {
    const now: BigNumber = await latest();
    token = await makeSUT();
    await token.initAntibot(now.add(3 * 24 * 60 * 60), getBigNumber(50000));
  });
  ...
  describe("onlyOwner", () => {
    it("should revert if restricted function's caller is not owner", async () => {
      ...
      await expect(token.connect(alice).initAntibot(1, getBigNumber(50000))).to.be.revertedWith("caller is not the owner");
    });
  });
  ...
  describe("initAntibot", () => {
    ...
    it("should revert if already initialized", async () => {
      await expect(token.initAntibot(1, getBigNumber(50000))).to.be.revertedWith("Protection: Already initialized");
    });
  });
